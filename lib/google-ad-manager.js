...
async function withRetry(fn, { attempts = 3, label = 'op' } = {}) {
  ...
  for (let i = 0; i < attempts; i++) {
    try {
      ...
      lastErr = err;
      const retriable =
        /ECONN|ETIMEDOUT|ENETUNREACH|ENOTFOUND|EAI_AGAIN|socket hang up|HTTP (401|5\d\d|429|408)|token refresh needed|aborted|timeout/i.test(
...
async function callSoap(service, networkCode, operationBody, { label = service, timeoutMs = 15_000, attempts = 2 } = {}) {
  ...
  const res = await withRetry(async () => {
    ...
    // 401이면 토큰 캐시 비우고 (다음 시도 때 새 토큰으로)
    if (r.status === 401) {
      _tokenCache = null;
      throw new Error('HTTP 401 (token refresh needed)');
    }
    return r;
  }, { attempts, label });
...
async function runReport({ networkCode, dimensions, columns, startDate, endDate, adUnitView = 'FLAT' }) {
  ...
  // 2) 상태 폴링 — 공격적 초기 폴링
  //    간격: 400ms → 600ms → 900ms → 1200ms → 1500ms → 2000ms (최대)
  //    상태 체크는 재시도 없이 1회만 (실패 시 다음 폴링에서 자연스럽게 재확인)
  const deadline = Date.now() + 50_000; // 50초 넘기면 포기 (vercel.json maxDuration=60 과 버퍼)
  const intervals = [400, 600, 900, 1200, 1500, 2000];
  let attempt = 0;
  ...
  while (Date.now() < deadline) {
    ...
    let statusXml;
    try {
      statusXml = await callSoap('ReportService', networkCode, statusBody, {
        label: `getReportJobStatus#${attempt}`,
        timeoutMs: 8_000,
        attempts: 1, // 상태 체크는 재시도 안 함 (다음 iteration 에서 자연 재시도)
      });
    } catch (e) {
      // 상태 체크 실패해도 계속 폴링 (다음 iteration 시도)
      console.warn(`[google] status poll #${attempt} failed, continuing: ${e.message}`);
      continue;
    }
